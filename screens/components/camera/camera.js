...
export default function CameraITSY({ isCameraOn, offCamera,onTakePhoto}) {
 ...
 const [photoPrev,setPhotoPrev] = useState(null)
  ...
  async function takePicture() {
    if (cameraRef) {
      ...
      onTakePhoto(resizedPhoto.uri)
      setPhotoPrev(resizedPhoto.uri)
  ...
  return (
    <View className=" flex relative flex-1  h-full w-full  ">
      <View className=" bg-background w-full relative flex-[.1] flex flex-row items-center justify-between px-5">
        ...
        <Image source={ITSYLogo} className=" h-14 w-14 rounded-md" />

        <TouchableOpacity className="thunder" onPress={()=>{
